...
- bc3 Property with two preceding underscore shouldn't be accessed from any \
...
- bc15 Use the area statement syntax to structure your code and make it \
       ...
       below). If you are forced to indent an area nearer to left border as \
       in the logic of meta structuring use one "#" for each less of indention.

- bc16 If a module could offer a usable command line interface to provide \
       their functionality directly for other programs (maybe not written in \
       python) implement the "Runnable" interface from \
...
header in level "j" if "j < i" holds. If your try to import a module from a \
higher level ("j < i") you could try to use the "from ... import ..." \
statement in the needed context dependent scope.
