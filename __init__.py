...
Conventions (LCx := library convention number x)
    ...
    - lc4 Follow the python OOP conventions like camel-case class-names or
          underscore separated methods and property names.
    - lc5 Class-names have a leading upper case letter.
    - lc6 Methods and functions are complete lower case.
    - lc7 Do not use more chars then 79 in one line.
    - lc8 Use short and/or long description doc-strings for all definitions.
    - lc9 Write doctest for each unit it is possible and try to reach 100% path
          coverage.
    - lc10 Every temporary generated file (e.g. for mocups) should have its
           name prefixed with "temp_" to support automatic clean up after
           running automated test cases.
    - lc11 Sorting imports as following:
    ...
    - lc12 Import everthing by its whole name and reference path and use it by
           its full reference path (even builtin units).
    - lc13 Don't use any abbreviations.
    - lc14 Follow the pep8 standards.
    - lc15 Try to use small cyclomatice complexity in all units.
           (e.g. less than 20 or 30).
    - lc16 Use the modules pattern described below.
    - lc17 Use the area statement syntax to structure your code and make it
           possible to fold them in many IDE's
           (see Structure of meta documenting below).
    - lc18 If a module could offer a usable command line interface to provide
           their functionality directly for other programs
           (maybe not written in python) implement the "Runnable" interface
           from "library.extension.system.Runnable".
    - lc19 Make every script or package standalone runnable. That means you
           should use relative import references or append your own import path
           dynamicly generated.
    - lc20 Always think that code is more read than written.
    - lc21 By choosing witch quotes to use follow this priority.
               1. Single quote (')
               2. Double quote (")
               3. Triple single quote (''')
               4. Triple double quote (three times ")
    - lc22 Indent function parameter which doesn't match in one line like:
...
Structure of dependencies
    ...
    0. builtins
    1. library.extension.dependent
    1. library.extension.type
    2. library.aspect.signature
    3. library.paradigm.aspectOrientation
    4. library.paradigm.objectOrientation
    5. library.extension.native
    6. library.extension.file
    7. library.extension.output
    8. library.extension.system
    9. library.*
...
Module pattern (see lc16)
    ...
    library.extension.system.CommandLine.package(
...
import library.aspect.signature
import library.extension.system
...
'''
    ...
    this package.
'''
library.aspect.signature.add_check(
    point_cut='^%s\..*$' % library.extension.native.Module.get_package_name(
        frame=inspect.currentframe()))

# region footer

library.extension.system.CommandLine.generic_package_interface(
    name=__name__, frame=inspect.currentframe())
